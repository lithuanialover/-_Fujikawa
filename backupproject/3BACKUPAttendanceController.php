...
class AttendanceController extends Controller
{
  ...
  public function getIndex(Request $request)
  {
    ...
    /*ボタンが正常に作動しない原因
    *if ($attendance != null)を使っているから
    いきなり、ボタンの条件 true falseを書いたら解決する
    だから回答に合わせて書き換えたほうがよい
    */
    if ($attendance != null) { //勤務データがある場合

      if ($attendance['end_time'] != '00:00:00') {
...
$rest = Rest::where('user_id', $user_id)->where('date', $today)->orderBy('start_rest', 'desc')->first();
  ...
  {
    $items = Attendance::Paginate(4);

    return view('/attendance/{num}', ['items' => $items]);
  }
}
